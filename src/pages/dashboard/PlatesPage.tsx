...
export default function PlatesPage() {
  ...
  async function deletePlate(plateId: string) {
    if (!confirm('Supprimer ce support ? Les scans associés seront perdus.')) return
    await supabase.from('plates').delete().eq('id', plateId)
    loadPlates()
  }

  async function unclaimPlate(plate: Plate & { scan_count?: number }) {
    if (!confirm(`Désaffilier "${plate.label || plate.code}" ? Le tag redeviendra vierge et pourra être réactivé sur un autre compte.`)) return
    const { data, error } = await supabase.rpc('unclaim_plate', { plate_code: plate.code })
        ...
        <div style={{ display:'flex', alignItems:'center', justifyContent:'space-between' }}>
          <div style={{ display:'flex', alignItems:'center', gap:12, flex:1, minWidth:0 }}>
            <div style={{ width:40, height:40, borderRadius:10, background: plate.plate_type==='nfc'?'rgba(37,99,235,0.06)':'rgba(124,58,237,0.06)', display:'flex', alignItems:'center', justifyContent:'center', flexShrink:0 }}>
              {plate.plate_type === 'nfc'
                ? <Wifi size={18} color="#2563eb" strokeWidth={1.5} />
                : <QrCode size={18} color="#7c3aed" strokeWidth={1.5} />
              }
            </div>
          ...
          {!isEditing && (
            <div style={{ display:'flex', alignItems:'center', gap:2, flexWrap:'wrap', justifyContent:'flex-end' }}>
              ...
              <button onClick={() => setPreviewPlate(plate)} style={iconBtn} title="Prévisualiser">
              ...
              <button onClick={() => downloadQr(plate)} style={iconBtn} title="Télécharger QR">
              ...
              {plate.plate_type === 'nfc' && plate.establishment_id && (
                <button onClick={() => unclaimPlate(plate)} style={{ ...iconBtn, color:'#f59e0b' }} title="Désaffilier ce tag">
                  <svg width="16" height="16" fill="none" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round"><path d="M18.36 6.64A9 9 0 0 1 20.77 15M2 12a10 10 0 0 1 18-6M2 12a10 10 0 0 0 18 6M12 2v4M12 18v4" transform="scale(0.67)"/></svg>
                </button>
              )}
        ...
        <div style={{ display:'flex', flexDirection:'column', gap:32 }}>
          <section>
            ...
            <div style={{ background:'#eff6ff', borderRadius:10, padding:12, marginBottom:12 }}>
              <p style={{ fontSize:12, color:'#1e40af', margin:0 }}>Vos tags NFC encodés arrivent bientôt. Ils apparaîtront ici après le premier scan.</p>
            </div>
            {nfcPlates.length === 0 ? (
